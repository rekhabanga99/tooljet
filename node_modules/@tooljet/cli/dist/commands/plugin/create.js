...
class Create extends core_1.Command {
    async run() {
        ...
        const name = await core_1.CliUx.ux.prompt('Enter plugin display name');
        if (Number(name)) {
        ...
        if (!(fs.existsSync(pluginsPath) && fs.existsSync(docsPath) && fs.existsSync(defaultTemplates))) {
            this.log('\x1b[41m%s\x1b[0m', `Error : ${pluginsPath}, docs or ${pluginsPath}/_templates directory missing, make sure that you are runing this command in Tooljet directory`);
            process.exit(1);
        }
        let repoUrl;
        if (marketplace) {
            ...
            pluginsJson.map((plugin) => {
                if (plugin.id === args.plugin_name.toLowerCase()) {
                    this.log('\x1b[41m%s\x1b[0m', 'Error : Plugin id already exists');
                    process.exit(1);
                }
            });
            repoUrl = await core_1.CliUx.ux.prompt('Please enter the repository URL if hosted on GitHub', {
                required: false,
            });
        }
        const hygenArgs = [
        ...
        core_1.CliUx.ux.action.start('creating plugin');
        ...
        const subtree = core_1.CliUx.ux.tree();
...
Create.flags = {
    type: core_1.Flags.string({ options: ['database', 'api', 'cloud-storage'] }),
    build: core_1.Flags.boolean({ char: 'b' }),
    marketplace: core_1.Flags.boolean({ char: 'm' }),
};
Create.description = 'Create a new tooljet plugin';
Create.examples = [`$ tooljet plugin create <name> --type=<database | api | cloud-storage> [--build]`];
Create.args = [{ name: 'plugin_name', description: 'Name of the plugin', required: true }];
